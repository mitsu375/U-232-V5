...
$HTMLOUT.= "<h4>Snatches for torrent <a href='{$INSTALLER09['baseurl']}/details.php?id=" . (int)$arr['id'] . "'>" . htmlsafechars($arr['name']) . "</a></h4>\n";
$HTMLOUT.= "<h5>Currently {$row['0']} snatch" . ($row[0] == 1 ? "" : "es") . "</h5>\n";
if ($count > $perpage) $HTMLOUT.= $pager['pagertop'];
$HTMLOUT.= "<table class='table table-bordered'>
<tr>
<td class='colhead' align='left'>{$lang['snatches_username']}</td>
<td class='colhead' align='center'>{$lang['snatches_uploaded']}</td>
" . ($INSTALLER09['ratio_free'] ? "" : "<td class='colhead' align='center'>{$lang['snatches_downloaded']}</td>") . "
<td class='colhead' align='center'>{$lang['snatches_ratio']}</td>
<td class='colhead' align='center'>{$lang['snatches_seedtime']}</td>
<td class='colhead' align='center'>{$lang['snatches_leechtime']}</td>
...
<td class='colhead' align='center'>{$lang['snatches_completed']}</td>
</tr>\n";
...
while ($arr = mysqli_fetch_assoc($res)) {
    ...
    $active = ($arr['active'] == 1 ? $active = "<img src='" . $INSTALLER09['pic_base_url'] . "aff_tick.png' alt='Yes' title='Yes' />" : $active = "<img src='" . $INSTALLER09['pic_base_url'] . "aff_cross.gif' alt='No' title='No' />");
    $completed = ($arr['completed'] >= 1 ? $completed = "<img src='" . $INSTALLER09['pic_base_url'] . "aff_tick.png' alt='Yes' title='Yes' />" : $completed = "<img src='" . $INSTALLER09['pic_base_url'] . "aff_cross.gif' alt='No' title='No' />");
  ...
  " . ($INSTALLER09['ratio_free'] ? "" : "<td align='center'>" . mksize($arr["downloaded"]) . "</td>") . "
  <td align='center'>" . htmlsafechars($ratio) . "</td>
  <td align='center'>" . mkprettytime($arr["seedtime"]) . "</td>
  <td align='center'>" . mkprettytime($arr["leechtime"]) . "</td>
  <td align='center'>" . get_date($arr["mtime"], '', 0, 1) . "</td>
  <td align='center'>" . (int)$arr["announced"] . "</td>
