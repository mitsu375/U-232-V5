...
if ($url == '' && !empty($descr)) {
    preg_match('{\b(tt[0-9]{0,8})\b}i', $descr, $text);
    if (!empty($text) && $text[0] != "") {
    $url = 'https://www.imdb.com/title/' . $text[0];
    }
}
$catid = (0 + $_POST["type"]);
...
$ret = sql_query("INSERT INTO torrents (search_text, filename, owner, username, visible, checked_when, checked_by, vip, release_group, newgenre, poster, anonymous, allow_comments, info_hash, name, size, numfiles, type, offer, request, url, subs, descr, ori_descr, description, category, free, silver, save_as, youtube, tags, added, last_action, mtime, ctime, freetorrent, nfo, client_created_by) VALUES (" . implode(",", array_map("sqlesc", array(
...
if ($INSTALLER09['seedbonus_on'] == 1) {
    if (isset($_POST['uplver']) && $_POST['uplver'] == 'yes')
        ...
        $messages = "\0035[".htmlsafechars($Cat_Name['cat_name'])."]\003 \00310$torrent\003 - {$INSTALLER09['baseurl']}/details.php?id=$id -\0034 " . mksize($totallen) . " \003";
    //===add karma
